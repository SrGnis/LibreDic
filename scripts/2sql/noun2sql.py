...
with open('./sql/inserts/nouns.sql', 'w') as of:
    ...
    for p in todo:
        ...
        of.write('((SELECT MAX(id) FROM sentidos),(SELECT (id) FROM categorias WHERE tipo_categoria=\'N\'));\n')
        ...
        of.write('INSERT INTO propiedades (id_tipo_propiedad, valor, id_caracteristicas) VALUES\n')
        of.write('((SELECT (id) FROM tipos_propiedades WHERE tipo_propiedad=\'NC\'),(\'{valor}\') ,(SELECT MAX(id) FROM caracteristicas));\n'.format(valor=p[6]))

        of.write('INSERT INTO propiedades (id_tipo_propiedad, valor, id_caracteristicas) VALUES\n')
        of.write('((SELECT (id) FROM tipos_propiedades WHERE tipo_propiedad=\'NSC\'),(\'{valor}\') ,(SELECT MAX(id) FROM caracteristicas));\n'.format(valor=p[7]))
