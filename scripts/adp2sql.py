...
with open('./data/csv/adp.csv', newline='') as csvfile:
...
with open('./sql/inserts/adp_words.sql', 'w') as of:
    first=True
...
with open('./sql/inserts/adp.sql', 'w') as of:
    conta=0
    for p in todo:
        #Sentidos
        ...
        of.write('((SELECT MAX(id) FROM sentidos),(SELECT (id) FROM categorias WHERE tipo_categoria=\'S\'));\n')
